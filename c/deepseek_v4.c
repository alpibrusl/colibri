...
static int moe_token(float *output,
     ...
     * when routing succeeded -- indices is uninitialised otherwise. Placed
     * AFTER the #endif so the capture does not depend on the build config. */
...
static int v4_serve_one(ColiV4Engine *engine, ColiV4Session *session,
    ...
    if (gw_parse(request->prompt, request->prompt_bytes, &gtx, &gtl, &gsp, &gns) < 0) {
        v4_serve_error(stdout, request->id, "bad guard table");
        return 0;
    }
